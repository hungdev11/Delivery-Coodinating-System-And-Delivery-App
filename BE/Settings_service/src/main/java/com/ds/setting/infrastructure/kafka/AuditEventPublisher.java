...
import com.ds.setting.infrastructure.kafka.dto.AuditEventDto;
...
public class AuditEventPublisher {
    ...
    public void publishAuditEvent(AuditEventDto event) {
        try {
            ...
            if (event.getSourceService() == null || event.getSourceService().isBlank()) {
                event.setSourceService("settings-service");
            }
            ...
            future.whenComplete((result, ex) -> {
                if (ex == null) {
                    log.debug("[settings-service] [AuditEventPublisher.publishAuditEvent] Audit event published successfully: eventId={}, operation={}, resourceType={}", 
                        event.getEventId(), event.getOperationType(), event.getResourceType());
                } else {
                    log.error("[settings-service] [AuditEventPublisher.publishAuditEvent] Failed to publish audit event: eventId={}, operation={}, resourceType={}", 
            ...
            log.error("[settings-service] [AuditEventPublisher.publishAuditEvent] Error publishing audit event: {}", e.getMessage(), e);
    ...
    private void sendToDeadLetterQueue(AuditEventDto event, Throwable error) {
        try {
            ...
            log.debug("[settings-service] [AuditEventPublisher.sendToDeadLetterQueue] Audit event sent to DLQ: eventId={}", event.getEventId());
            
        } catch (Exception e) {
            log.error("[settings-service] [AuditEventPublisher.sendToDeadLetterQueue] CRITICAL: Failed to send audit event to DLQ", e);
