...
public class SettingsInitializationService
        implements ISettingsInitializationService, ISettingsWriterService, ISettingsReaderService {
    ...
    @PostConstruct
    public void init() {
        log.debug("SettingsInitializationService initialized with:");
        log.debug("  - Settings Service URL: {}", settingsServiceUrl);
        log.debug("  - Max Retries: {}", maxRetries);
        log.debug("  - Retry Delay: {} seconds", retryDelaySeconds);
    }
    ...
    public void initialize() {
        log.debug("Checking Keycloak Settings in Settings Service");

        // Wait for Settings Service to be available
        if (!waitForSettingsService()) {
            log.error(
                    "Settings Service is not available after maximum retry attempts. Continuing without settings initialization.");
            return;
        }

        try {
            if (isSettingsAlreadyInitialized()) {
                log.debug("Keycloak settings already initialized - skipping");
            } else {
                log.debug("Initializing Keycloak settings...");
                initializeKeycloakSettings();
                log.debug("Keycloak settings initialized successfully");
            }
        } catch (Exception e) {
            log.debug("[user-service] [SettingsInitializationService.checkAndInitializeSettings] Failed to check/initialize settings: {}", e.getMessage());
        }
    }
    ...
    private boolean waitForSettingsService() {
        ...
        log.debug("Waiting for Settings Service to be available...");

        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                log.debug("Attempt {}/{}: Checking Settings Service availability at {}",
                ...
                if (isAvailable) {
                    log.debug("Settings Service is available after {} attempts", attempt);
                    return true;
                }

            } catch (Exception e) {
                log.debug("[user-service] [SettingsInitializationService.waitForSettingsService] Attempt {}/{} failed: Settings Service not available - {}",
                        attempt, maxRetries, e.getMessage());

                if (attempt < maxRetries) {
                    log.debug("Waiting {} seconds before next attempt...", retryDelaySeconds);
                    try {
    ...
    private void initializeKeycloakSettings() {
        ...
        addSettingToList(settingsToUpsert, "KEYCLOAK_MASTER_REALM", "Keycloak master realm for admin operations",
                "STRING",
        ...
        addSettingToList(settingsToUpsert, "KEYCLOAK_DEFAULT_REALM", "Default realm for API Gateway authentication",
                "STRING",
                initConfig.getDefaultConfig().getRealm(), "SYSTEM", true, "TEXT");
        addSettingToList(settingsToUpsert, "KEYCLOAK_DEFAULT_CLIENT_ID",
                "Default client ID for API Gateway authentication", "STRING",
        ...
        for (KeycloakInitConfig.RealmConfig realmConfig : initConfig.getRealms()) {
            ...
            addSettingToList(settingsToUpsert, "KEYCLOAK_REALM_" + realmKey, "Realm: " + realmConfig.getDisplayName(),
                    "STRING",
            ...
            for (KeycloakInitConfig.ClientConfig clientConfig : realmConfig.getClients()) {
                ...
                addSettingToList(settingsToUpsert, "KEYCLOAK_CLIENT_" + clientKey + "_ID",
                        "Client ID for " + clientConfig.getName(),
    ...
    private void addSettingToList(List<Map<String, Object>> list, String key, String description,
        ...
        if (value == null || value.trim().isEmpty()) {
            log.debug("[user-service] [SettingsInitializationService.addSettingToList] Skipping creation of '{}' - value is null or empty", key);
            return;
        }
    ...
    private void bulkUpsertSettings(String group, List<Map<String, Object>> settings) {
        if (settings.isEmpty()) {
            log.debug("[user-service] [SettingsInitializationService.bulkUpsertSettings] No settings to upsert for group: {}", group);
            return;
        }
        ...
        try {
            ...
            log.debug("  Bulk upserted {} settings in group {}", settings.size(), group);
        } catch (Exception e) {
            log.error("[user-service] [SettingsInitializationService.bulkUpsertSettings] Failed to bulk upsert settings in group '{}'", group, e);
            // Fallback to individual upserts if bulk fails
            log.debug("[user-service] [SettingsInitializationService.bulkUpsertSettings] Falling back to individual upserts...");
    ...
    public void createSetting(String key, String group, String description,
        ...
        try {
            ...
            if (value == null || value.trim().isEmpty()) {
                log.debug("[user-service] [SettingsInitializationService.createSetting] Skipping creation of '{}' in group '{}' - value is null or empty", key, group);
                return;
            }
            ...
            HttpEntity<Map<String, Object>> entity = new HttpEntity<>(request, headers);
            restTemplate.put(url, entity);
            log.debug("  Created/Updated: {} in group {}", key, group);
        } catch (Exception e) {
            log.debug("[user-service] [SettingsInitializationService.createSetting] Failed to create/update '{}' in group '{}': {}", key, group, e.getMessage());
        }
    }
