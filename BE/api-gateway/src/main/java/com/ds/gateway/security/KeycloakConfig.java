 ...
 * Configures JWT decoder with custom validation
 */
@Slf4j
@Configuration
public class KeycloakConfig {
     ...
     * - Validates issuer
     * - Validates timestamp (expiration)
     */
    @Bean
            ...
            try {
                if (jwt.getExpiresAt() != null && jwt.getExpiresAt().isBefore(java.time.Instant.now())) {
                    return OAuth2TokenValidatorResult.failure(
                        new OAuth2Error("jwt_expired", "JWT token has expired", null)
                    );
                }
                return OAuth2TokenValidatorResult.success();
            } catch (Exception e) {
                return OAuth2TokenValidatorResult.failure(
                    new OAuth2Error("jwt_validation_error", "JWT validation failed", null)
                );
            }
        }
    }
