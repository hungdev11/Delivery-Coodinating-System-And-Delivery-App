...
import com.ds.session.session_service.infrastructure.kafka.dto.AuditEventDto;
...
public class AuditEventPublisher {
    ...
    public void publishAuditEvent(AuditEventDto event) {
        try {
            ...
            if (event.getSourceService() == null || event.getSourceService().isBlank()) {
                event.setSourceService("session-service");
            }
            ...
            future.whenComplete((result, ex) -> {
                if (ex == null) {
                    log.debug("[session-service] [AuditEventPublisher.publishAuditEvent] Audit event published successfully: eventId={}, operation={}, resourceType={}", 
                        event.getEventId(), event.getOperationType(), event.getResourceType());
                } else {
                    log.error("[session-service] [AuditEventPublisher.publishAuditEvent] Failed to publish audit event: eventId={}, operation={}, resourceType={}", 
            ...
            log.error("[session-service] [AuditEventPublisher.publishAuditEvent] Error publishing audit event: {}", e.getMessage(), e);
    ...
    private void sendToDeadLetterQueue(AuditEventDto event, Throwable error) {
        try {
            ...
            log.debug("[session-service] [AuditEventPublisher.sendToDeadLetterQueue] Audit event sent to DLQ: eventId={}", event.getEventId());
            
        } catch (Exception e) {
            log.error("[session-service] [AuditEventPublisher.sendToDeadLetterQueue] CRITICAL: Failed to send audit event to DLQ", e);
