 ...
 * Downloads full user/delivery man dump from UserService if snapshot tables are empty
 */
@Service
    ...
    public void doInitializeSnapshot() {
        ...
        try {
            ...
            if (!needUserInit && !needDeliveryManInit) {
                log.info("✅ Snapshot tables already populated (users: {}, deliveryMen: {}), skipping initialization", 
                    userSnapshotCount, deliveryManSnapshotCount);
                return;
            }

            log.info("🔄 Starting snapshot initialization... (users: {}, deliveryMen: {})", 
            ...
            if (needUserInit) {
                ...
                log.info("✅ User snapshot initialization completed: {} users loaded", totalLoaded);
            }
            
            // Load delivery men if needed
            if (needDeliveryManInit) {
                int totalLoaded = loadAllDeliveryMenFromDump();
                log.info("✅ DeliveryMan snapshot initialization completed: {} delivery men loaded", totalLoaded);
            }
    ...
    private int loadAllDeliveryMenFromDump() {
        ...
        while (hasMore) {
            try {
                ...
                log.info("📥 Downloading delivery man dump: page={}, size={}", page, pageSize);
                ...
                if (deliveryMen == null || deliveryMen.isEmpty()) {
                    log.warn("⚠️ No delivery men returned in dump response");
                    break;
                }
                ...
                for (Map<String, Object> dmData : deliveryMen) {
                    DeliveryManSnapshot snapshot = DeliveryManSnapshot.builder()
                        ...
                        .capacityKg(dmData.get("capacityKg") != null ? 
                            Double.parseDouble(dmData.get("capacityKg").toString()) : null)
                ...
                log.info("✅ Loaded {} delivery men (total: {})", snapshots.size(), totalLoaded);
