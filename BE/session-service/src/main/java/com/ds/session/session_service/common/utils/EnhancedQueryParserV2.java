...
import com.ds.session.session_service.common.entities.dto.filter.v2.*;
...
public class EnhancedQueryParserV2 {
    ...
    private static <T> Predicate parseFilterCondition(Root<T> root, CriteriaBuilder criteriaBuilder, 
        ...
        try {
            ...
            if (fieldPath == null) {
                log.debug("[session-service] [EnhancedQueryParserV2.parseCondition] Field path not found: {}", field);
                return null;
            }
            ...
            switch (operator) {
                ...
                case ENDS_WITH:
                    return criteriaBuilder.like(fieldPath.as(String.class), "%" + value);
                default:
                    log.debug("[session-service] [EnhancedQueryParserV2.parseCondition] Unsupported operator: {}", operator);
                    return null;
            }
            
        } catch (Exception e) {
            log.error("[session-service] [EnhancedQueryParserV2.parseCondition] Error parsing filter condition", e);
            return null;
        }
    }
    ...
    private static <T> Path<?> getFieldPath(Root<T> root, String field) {
        try {
            ...
            log.debug("[session-service] [EnhancedQueryParserV2.getFieldPath] Error getting field path for: {}, error: {}", field, e.getMessage());
            return null;
        }
    }
}
