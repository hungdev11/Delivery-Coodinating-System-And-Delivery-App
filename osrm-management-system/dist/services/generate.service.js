...
class GenerateService {
    ...
    constructor(prisma) {
        ...
        this.rawDataPath = (0, path_1.resolve)(process.env.RAW_DATA_PATH || './raw_data');
    ...
    generateBicycleLuaProfile(config) {
        const { useRating, useBlocking } = config;
        // Read bicycle.lua base from zone_service
        const bicycleLuaPath = (0, path_1.join)(this.rawDataPath, 'osrm-logic', 'bicycle.lua');
        if (!(0, fs_1.existsSync)(bicycleLuaPath)) {
            logger_1.logger.warn(`bicycle.lua not found at ${bicycleLuaPath}, using fallback`);
            return this.generateBicycleLuaProfileFallback(config);
        }
        let bicycleLua = (0, fs_1.readFileSync)(bicycleLuaPath, 'utf-8');
        ...
        bicycleLua = bicycleLua.replace(/result\.traffic_lights\s*=\s*TrafficSignal\.get_value\(node\)/g, 'result.traffic_lights = highway == "traffic_signals"');
        // 3. Add modifiers to setup() - find avoid Set closing and inject before final }
        bicycleLua = bicycleLua.replace(/(avoid\s*=\s*Set\s*\{[^}]*\})(\n\s*\})/, `$1,\n\n    use_rating = ${useRating},\n    use_blocking = ${useBlocking}$2`);
...
`;
        ...
        if (runPattern.test(bicycleLua)) {
            ...
            logger_1.logger.warn('Failed to inject modifiers after WayHandlers.run() in bicycle.lua - pattern not matched');
        }
        // Add header
...
`;
    ...
    generateBicycleLuaProfileFallback(config) {
        const { useRating, useBlocking } = config;
        logger_1.logger.warn('Using fallback bicycle profile (hardcoded)');
...
`;
    ...
    generateCarLuaProfile(config) {
        const { useRating, useBlocking } = config;
        // Read car.lua base from zone_service
        const carLuaPath = (0, path_1.join)(this.rawDataPath, 'osrm-logic', 'car.lua');
        if (!(0, fs_1.existsSync)(carLuaPath)) {
            logger_1.logger.warn(`car.lua not found at ${carLuaPath}, using fallback`);
            return this.generateCarLuaProfileFallback(config);
        }
        let carLua = (0, fs_1.readFileSync)(carLuaPath, 'utf-8');
        ...
        carLua = carLua.replace(/(access_turn_classification\s*=\s*\{\s*\n\s*\})(\n\s*\})/, `$1,\n\n    use_rating = ${useRating},\n    use_blocking = ${useBlocking}$2`);
        // 3. Fix process_node - remove obstacle_map calls and Obstacles.process_node
        // Replace obstacle_map:add with result.barrier = true
        carLua = carLua.replace(/obstacle_map:add\(node, Obstacle\.new\(obstacle_type\.barrier\)\)/g, 'result.barrier = true');
...
`;
        ...
        if (relationsPattern.test(carLua)) {
            ...
            if (runPattern.test(carLua)) {
                ...
                logger_1.logger.warn('Failed to inject modifiers in car.lua - pattern not matched');
            }
        }
        ...
        carLua = carLua.replace(/for\s+_,\s+obs\s+in\s+pairs\(obstacle_map:get\(turn\.from, turn\.via\)\)\s+do[\s\S]*?::skip::[\s\S]*?end\s*\n/g, '');
...
`;
    ...
    generateCarLuaProfileFallback(config) {
        const { useRating, useBlocking } = config;
        logger_1.logger.warn('Using fallback car profile (hardcoded)');
...
`;
    ...
    async setupModelInstance(config, sharedXmlPath) {
        ...
        const luaPath = (0, path_1.join)(instanceDir, profileFileName);
        (0, fs_1.writeFileSync)(luaPath, luaScript, 'utf-8');
        logger_1.logger.info(`Setup ${config.name} (${config.vehicleType})`);
    }
    async processOSRMInstance(config) {
        logger_1.logger.info(`Processing ${config.name} (${config.vehicleType})...`);
