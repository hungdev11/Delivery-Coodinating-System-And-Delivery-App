...
`;
        bicycleLua = bicycleLua.replace(/(WayHandlers\.run\(profile, way, result, data, handlers\)\s*\n)(end)/, `$1${modifiersCode}$2`);
        // Add header
...
`;
        carLua = carLua.replace(/(if profile\.cardinal_directions then\s*\n\s*Relations\.process_way_refs\(way, relations, result\)\s*\n\s*end\s*\n)(end)/, `$1${modifiersCode}$2`);
