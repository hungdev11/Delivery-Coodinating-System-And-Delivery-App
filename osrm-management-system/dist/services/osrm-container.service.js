...
exports.OSRMContainerService = void 0;
...
class OSRMContainerService {
    osrmDataPath;
    dockerImage;
    composeProjectName;
    composeFilePath;
    constructor() {
        this.osrmDataPath = (0, path_1.resolve)(process.env.OSRM_DATA_PATH || './osrm_data');
        ...
        this.composeFilePath = (0, path_1.resolve)(process.env.DOCKER_COMPOSE_FILE || './docker-compose.yml');
    }
    /**
    ...
    async getStatus() {
        ...
        for (const container of OSRM_CONTAINERS) {
            try {
                ...
                let health;
                try {
                    const { stdout: statusOutput } = await execAsync(`docker-compose -p ${this.composeProjectName} -f ${this.composeFilePath} ps ${container.name} --format json 2>&1`);
                    ...
                    try {
                        const { stdout: textOutput } = await execAsync(`docker-compose -p ${this.composeProjectName} -f ${this.composeFilePath} ps ${container.name} 2>&1`);
                        ...
                        const { stdout: dockerOutput } = await execAsync(`docker ps -a --filter "name=${container.containerName}" --format "{{.Status}}" 2>&1 || echo ""`);
    ...
    async startContainer(model) {
        ...
        try {
            ...
            await execAsync(`docker-compose -p ${this.composeProjectName} -f ${this.composeFilePath} up -d ${container.name} 2>&1`);
            logger_1.logger.info(`Started container: ${container.containerName} via docker-compose`);
            return {
                success: true,
            ...
            logger_1.logger.error(`Failed to start container ${container.containerName}`, { error: error.message });
            return {
                success: false,
                container: container.name,
                error: error.message,
    ...
    async stopContainer(model) {
        ...
        try {
            ...
            await execAsync(`docker-compose -p ${this.composeProjectName} -f ${this.composeFilePath} stop ${container.name} 2>&1`);
            logger_1.logger.info(`Stopped container: ${container.containerName} via docker-compose`);
            return {
                success: true,
                container: container.name,
            ...
            logger_1.logger.error(`Failed to stop container ${container.containerName}`, { error: error.message });
            return {
                success: false,
                container: container.name,
                error: error.message,
    ...
    async restartContainer(model) {
        ...
        try {
            ...
            await execAsync(`docker-compose -p ${this.composeProjectName} -f ${this.composeFilePath} restart ${container.name} 2>&1`);
            logger_1.logger.info(`Restarted container: ${container.containerName} via docker-compose`);
            return {
                success: true,
                container: container.name,
            ...
            try {
                ...
                logger_1.logger.error(`Failed to restart container ${container.containerName}`, { error: error.message });
                return {
                    success: false,
                    container: container.name,
                    error: error.message,
    ...
    async rebuildContainer(model) {
        ...
        try {
            ...
            try {
                await execAsync(`docker-compose -p ${this.composeProjectName} -f ${this.composeFilePath} rm -f ${container.name} 2>&1`);
            }
            catch (e) {
                // Ignore if container doesn't exist
            }
            // Create and start new container using docker-compose
            await execAsync(`docker-compose -p ${this.composeProjectName} -f ${this.composeFilePath} up -d --force-recreate ${container.name} 2>&1`);
            logger_1.logger.info(`Rebuilt container: ${container.containerName} via docker-compose`);
            return {
                success: true,
                container: container.name,
            ...
            logger_1.logger.error(`Failed to rebuild container ${container.containerName}`, { error: error.message });
            return {
                success: false,
                container: container.name,
                error: error.message,
    ...
    async healthCheck(model) {
        ...
        try {
            ...
            let isRunning = false;
            try {
                const { stdout } = await execAsync(`docker ps --filter "name=${container.containerName}" --format "{{.Names}}" 2>&1`);
...
exports.OSRMContainerService = OSRMContainerService;
//# sourceMappingURL=osrm-container.service.js.map