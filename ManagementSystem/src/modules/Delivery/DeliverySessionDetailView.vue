...
const LazyShipperSessionsModal = defineAsyncComponent(
...
import { getProofsByAssignment, type DeliveryProofDto } from './api'
...
const loadRoute = async () => {
  ...
  // Starting point (warehouse) - fixed coordinates
  const startingPoint: MapMarker = {
    id: 'warehouse-start',
    coordinates: [106.79611036, 10.82398098], // [lon, lat]
    type: 'warehouse',
    label: 'Kho hàng',
    title: 'Kho hàng (Starting Point)',
    popup: 'Kho hàng - Điểm xuất phát',
    color: '#16a34a',
  }

  const visitOrder = result.visitOrder ?? []
  const deliveryMarkers: MapMarker[] = visitOrder.map((visit: VisitOrder, index: number) => {
    const waypoint = visit.waypoint ?? visit
    return {
      id: `waypoint-${index}`,
      coordinates: [waypoint.lon, waypoint.lat],
      type: 'delivery',
      label: visit.priorityLabel ?? `Điểm ${index + 1}`,
      title: visit.priorityLabel ?? `Điểm ${index + 1}`,
      popup: `Parcel: ${waypoint.parcelId ?? 'N/A'}<br/>Priority: ${visit.priority ?? '-'}`,
      color: '#1d4ed8',
    }
  })

  // Combine starting point with delivery markers (starting point first)
  mapMarkers.value = [startingPoint, ...deliveryMarkers]
...
/**
 * Open shipper sessions modal
 */
const openShipperSessionsModal = async () => {
  if (!session.value?.deliveryMan || !session.value.deliveryManId) return

  const dm = session.value.deliveryMan as any
  // Convert to DeliveryManDto format (only required fields for modal)
  const shipper = {
    id: session.value.deliveryManId, // Use deliveryManId as id
    userId: session.value.deliveryManId,
    vehicleType: dm.vehicleType || '',
    capacityKg: dm.capacityKg || 0,
    createdAt: '',
    updatedAt: '',
    displayName: dm.name || dm.displayName || 'N/A',
    email: dm.email || '',
    phone: dm.phone || '',
  } as import('./model.type').DeliveryManDto

  const modal = overlay.create(LazyShipperSessionsModal)
  const instance = modal.open({ shipper })
...
/**
 * Format date for display (consistent with modal)
 */
...
<template>
  <div class="container mx-auto px-4 py-6 space-y-6">
    ...
    >
      <template #actions>
        <UButton
          v-if="isAdmin && session?.deliveryMan"
          variant="solid"
          color="primary"
          icon="i-heroicons-list-bullet"
          @click="openShipperSessionsModal"
        >
          View All Sessions
        </UButton>
