...
/**
 * Sorting Select Modal for Android
 *
 * Allows multiple column selection with order control
 */
...
<template>
  <UModal :model-value="modelValue" @update:model-value="emit('update:modelValue', $event)">
    <template #header>
      <h3 class="text-lg font-semibold">Sắp xếp</h3>
    </template>
    ...
    <template #body>
      <div class="space-y-4">
        ...
        <div v-if="selectedSorts.length > 0" class="space-y-3">
          <h4 class="text-base font-semibold text-gray-700 dark:text-gray-300">Thứ tự sắp xếp:</h4>
          <div class="space-y-3">
            <div
            ...
            >
              <div
                class="flex-shrink-0 w-8 h-8 rounded-full bg-primary-500 text-white flex items-center justify-center text-sm font-semibold"
              >
                {{ sort.order }}
              </div>
        ...
        <div v-if="availableColumns.length > 0" class="space-y-3">
          <h4 class="text-base font-semibold text-gray-700 dark:text-gray-300">Chọn cột:</h4>
          <USelect
            :model-value="undefined"
            :items="availableColumns.map((col) => ({ label: col.label, value: col.id } as SelectItem))"
        ...
        <div v-else-if="selectedSorts.length === 0" class="text-center py-12 text-gray-500">
          <p class="text-base">Chưa có cột nào được chọn</p>
        </div>
      </div>
    </template>
    ...
    <template #footer>
      <div class="flex justify-between gap-2">
        ...
        <div class="flex gap-2">
          <UButton variant="outline" @click="handleCancel"> Hủy </UButton>
          <UButton @click="handleApply"> Áp dụng </UButton>
        </div>
      </div>
    </template>
  </UModal>
